...
- **`app/`** — rotas e layouts da aplicação: `/` (landing, route group `(landing)`), `/admin/*` (painel administrativo) e `/app/*` (área do aluno). Inclui `proxy.ts` na raiz de `src/`.
...
```
src/
├── proxy.ts            # Proteção de rotas por role (SPEC-frontend.md:89)
...
- `src/proxy.ts` (proteção de rotas por role, autorização no servidor): SPEC-frontend.md:89.

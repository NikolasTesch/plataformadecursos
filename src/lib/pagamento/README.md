...
- **Webhooks com HMAC (E2E-P3)**: chamada ao endpoint de webhook sem assinatura HMAC válida responde **401 e nenhum estado é alterado** (SPEC-pagamentos.md:119-121).
- **Idempotência (E2E-P4)**: `payment.approved` entregue 2x cria o entitlement uma única vez — chave de idempotência registrada em `purchases.mp_payment_id` (unique).
- Modelo de cobrança: 1 assinatura com 2 períodos (mensal/anual, preço anual configurável) + venda única por curso; cupons (`coupons`, `purchases.coupon_id`) e entitlements conforme [docs/modelo-de-dados.md](docs/modelo-de-dados.md) §2.6.
...
- Exemplos E2E (refund revoga entitlement, webhook inválido 401, duplicidade não duplica, trial único): [docs/specs/SPEC-pagamentos.md](docs/specs/SPEC-pagamentos.md):113-129.

...
- Armadilha: `auth/` e `rate-limit/` são consumidos por `src/services/` e pelas rotas finas; o middleware só importa `auth.config.ts` (Edge) — nunca `auth.ts` (Prisma não roda no Edge, BLOCKER-1).
